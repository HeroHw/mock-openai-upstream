...
> 监听端口固定为 `:9050`，不可配置。本地跑就是 `:9050`；容器里要换对外端口，改 docker-compose 的端口映射（见下文）。
...
# → [mockupstream] listening on :9050 (no auth — bind to internal network only)
...
curl localhost:9050/v1/chat/completions \
...
# Gemini 的 ?key= 查询参数
curl "localhost:9050/v1beta/models/gemini-pro:generateContent?key=sk-mock-secret" ...
```
...
curl 'localhost:9050/__mock/requests'

# 按条数和路径后缀过滤
curl 'localhost:9050/__mock/requests?limit=5&path_suffix=/chat/completions'

# 清空捕获记录（每个用例开始前清一次，断言才干净）
curl -X DELETE localhost:9050/__mock/requests
```
...
curl -s 'localhost:9050/__mock/requests?limit=1&path_suffix=/chat/completions' \
  | jq -r '.requests[0].body | fromjson | .temperature'

# 断言头剔除：X-Debug-Token 不应出现
curl -s 'localhost:9050/__mock/requests?limit=1' \
  | jq '.requests[0].headers | has("X-Debug-Token")'   # → false

# 断言透传：sha256 与客户端原始 body 一致
shasum -a 256 payload.json
curl -s 'localhost:9050/__mock/requests?limit=1' | jq -r '.requests[0].body_sha256'
```
...
curl -X POST localhost:9050/__mock/behavior \
...
curl localhost:9050/__mock/behavior
curl -X DELETE localhost:9050/__mock/behavior
```
...
docker run --rm -p 127.0.0.1:9050:9050 mockupstream:latest
...
要点：
- **监听端口固定为容器内 `:9050`**，不可配置。要换对外端口，改 `docker-compose.yml` 里 `ports` 的左侧宿主端口，例如 `"9000:9050"` 即可在 `localhost:9000` 访问；用 `docker run` 时改 `-p` 同理。
- **端口只绑定到宿主 loopback**（`127.0.0.1:9050:9050`），因为本服务默认不做认证，避免暴露到公网。
...
- 网关接入：与本服务同网络时把渠道 `BaseURL` 写 `http://mockupstream:9050`；从宿主机访问用 `http://localhost:9050`。

## Kubernetes 部署

`k8s/` 目录提供了完整清单（ConfigMap + Deployment + Service + 可选 NetworkPolicy），`kubectl apply -k k8s/` 一键部署。镜像推送、网关接入地址、单副本约束等注意事项见 [k8s/README.md](k8s/README.md)。

## 验证服务存活

```bash
curl localhost:9050/__mock/healthz
# {"status":"ok"}
```
